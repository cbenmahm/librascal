...
class FiniteTCalculator(GenericMDCalculator):
    ...
    def __init__(self, model_json, is_periodic, xdos, temperature, structure_template, atomic_numbers):
    ...
    def calculate(self, positions, cell_matrix):
        ...
        pred_dos = self.model.predict(self.manager)
        ...
        mu_T = getmu(pred_dos, self.beta, self.xdos, n=self.natoms)
        mu_0 = getmu(pred_dos, self.beta_0, self.xdos, n=self.natoms)
        ...
        weights_band_T = trapezoid(self.xdos * fd_T * self.dos_weights,
                                     self.xdos, axis=1)
        weights_band_0 = trapezoid(self.xdos * fd_0 * self.dos_weights,
                                     self.xdos, axis=1)
        
        # Compute the weights for the band energy forces at T>0 and T=0
        f_weights_band_T = trapezoid((self.xdos - shift_T) * fd_T * self.dos_weights,
                                     self.xdos, axis=1)
        f_weights_band_0 = trapezoid((self.xdos - shift_0) * fd_0 * self.dos_weights,
                                     self.xdos, axis=1)

        # Compute the weights for the entropy contribution at T>0 and T=0
        s_T, x_mask_T = get_entropy(fd_T)
        weights_entr_T = trapezoid(s_T * self.dos_weights[x_mask_T], self.xdos[x_mask_T], axis=1)
        weights_entr_T *= (-kB)

        s_0, x_mask_0 = get_entropy(fd_0)
        weights_entr_0 = trapezoid(s_0 * self.dos_weights[x_mask_0], self.xdos[x_mask_0], axis=1)
        weights_entr_0 *= (-kB)

        # Compute the weights for the entropy forces at T>0 and T=0
        f_weights_entr_T = weights_entr_T
        f_weights_entr_T += (kB * self.beta * (mu_T - shift_T) * trapezoid(fd_T * self.dos_weights, axis=1))

        f_weights_entr_0 = weights_entr_0
        f_weights_entr_0 += (kB * self.beta_0 * (mu_0 - shift_0) * trapezoid(fd_0 * self.dos_weights, axis=1))
        ...
        band_T_stress = self._extract_stress_matrix(band_T_stress_v)
        ...
        band_0_stress = self._extract_stress_matrix(band_0_stress_v)
        ...
        entr_T_stress = self._extract_stress_matrix(entr_T_stress_v)
        ...
        entr_0_stress = self._extract_stress_matrix(entr_0_stress_v)
